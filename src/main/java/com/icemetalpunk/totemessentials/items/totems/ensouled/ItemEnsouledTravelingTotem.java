...
public class ItemEnsouledTravelingTotem extends ItemEnsouledTotemBase {

	protected HashMap<Integer, String> dimensionNames = new HashMap<Integer, String>();

	public ItemEnsouledTravelingTotem(String name) {
		super(name);
		this.setMaxDamage(500); // 500 durability - 1 damage per 10 blocks =
								// 5000 blocks total
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
		...
		if (playerIn.isSneaking()) {
			String dimensionName = " - Unknown Dimension";
			if (dimensionNames.containsKey(Integer.valueOf(worldIn.provider.getDimension()))) {
				dimensionName = " - " + dimensionNames.get(Integer.valueOf(worldIn.provider.getDimension()));
			}
			...
			// Dimension change may have changed your position (coordinate
			// scaling), so get it fresh
			...
			if (destTag.length > 3 && destTag[3] != worldIn.provider.getDimension()) {
				playerIn.changeDimension(destTag[3]);
				dimensionCost = 10;
			}

			int maxDamage = (int) (1 + dimensionCost + Math.round(dist / 10.0d));
